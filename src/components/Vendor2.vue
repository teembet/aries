<template>
<div class="vendor">
  
      <div class="res-banner" >
         <h1 class="res-header">Resturant Name</h1>
         <p class="res-sub-header">Join lots other restaurants who benefit 
from having their menus on Aries</p>
     <v-rating readonly v-model = "rating"  color = "white" background-color="white">
     </v-rating>
  
     </div> 
    <div>
    <v-tabs
      v-model="active"
      color="white"
      slider-color="#f25d13"
      show-arrows
      fixed centered
      class="tabs"
    >
      <v-tab ripple><a  class="tab-item" href="#main-meals" v-smooth-scroll="{ duration: 1000, offset: -50 }">Main Meals</a> </v-tab>
       <v-tab ripple><a class="tab-item" href="#soup" v-smooth-scroll="{ duration: 1000, offset: -50 }">Soup</a></v-tab>
        <v-tab ripple><a  class="tab-item" href="#swallows" v-smooth-scroll="{ duration: 1000, offset: -50 }">Swallows</a></v-tab>
         <v-tab ripple><a class="tab-item" href="#sides" v-smooth-scroll="{ duration: 1000, offset: -50 }">Sides</a></v-tab>
          <v-tab ripple><a  class="tab-item" href="#drinks" v-smooth-scroll="{ duration: 1000, offset: -50 }">Drinks</a></v-tab>
    </v-tabs>
  </div>
  
 <v-layout row wrap>
        <v-flex xs12 sm6 md6>
           
         <v-card flat class="menu"> 
             <div class="vendor-header" id="main-meals"><strong>Main Meals</strong> </div>
        <v-list two-line>
          <div>
            <v-list-tile
            class="food-slot"
              :key="item.title"
              avatar
              ripple
              v-for="item in getMain">
              <v-list-tile-content>
                <v-list-tile-title class="food-name">{{ item.Name }}</v-list-tile-title>
                <v-list-tile-sub-title class="portion-text">₦{{ item.Price }}</v-list-tile-sub-title>
                
              </v-list-tile-content>
            
              <v-list-tile-action>
               
                 <v-btn fab dark color="#f25d13" depressed class="meal-btn" @click="Tobasket(item)">
                <p>+</p>
                </v-btn>
              </v-list-tile-action>

            </v-list-tile>
      
          </div>
        </v-list>
         <div class="vendor-header" id="soup"><strong>Soup</strong> </div>
       <v-list two-line>
          <div>
            <v-list-tile
            class="food-slot"
              :key="item.title"
              avatar
              ripple
              v-for="item in getSoup">
              <v-list-tile-content>
                <v-list-tile-title class="food-name">{{ item.Name }}</v-list-tile-title>
                <v-list-tile-sub-title class="portion-text">₦{{ item.Price }}</v-list-tile-sub-title>
                
              </v-list-tile-content>
            
              <v-list-tile-action>
               
                 <v-btn fab dark color="#f25d13" depressed class="meal-btn" @click="Tobasket(item)">
                <p>+</p>
                </v-btn>
              </v-list-tile-action>

            </v-list-tile>
      
          </div>
        </v-list>
         <div class="vendor-header" id="swallows"><strong>Swallows</strong> </div>
        <v-list two-line>
          <div>
            <v-list-tile
            class="food-slot"
              :key="item.title"
              avatar
              ripple
              v-for="item in getSwallows">
              <v-list-tile-content>
                <v-list-tile-title class="food-name">{{ item.Name }}</v-list-tile-title>
                <v-list-tile-sub-title class="portion-text">₦{{ item.Price }}</v-list-tile-sub-title>
                
              </v-list-tile-content>
            
              <v-list-tile-action>
               
                 <v-btn fab dark color="#f25d13" depressed class="meal-btn" @click="Tobasket(item)">
                <p>+</p>
                </v-btn>
              </v-list-tile-action>

            </v-list-tile>
      
          </div>
        </v-list>
        <div class="vendor-header" id="sides"><strong>Sides</strong> </div>
        <v-list two-line>
          <div>
            <v-list-tile
            class="food-slot"
              :key="item.title"
              avatar
              ripple
              v-for="item in getSides">
              <v-list-tile-content>
                <v-list-tile-title class="food-name">{{ item.Name }}</v-list-tile-title>
                <v-list-tile-sub-title class="portion-text">₦{{ item.Price }}</v-list-tile-sub-title>
                
              </v-list-tile-content>
            
              <v-list-tile-action>
               
                 <v-btn fab dark color="#f25d13" depressed class="meal-btn" @click="Tobasket(item)">
                <p>+</p>
                </v-btn>
              </v-list-tile-action>

            </v-list-tile>
      
          </div>
        </v-list>
        <div class="vendor-header" id="drinks"><strong>Drinks</strong> </div>
        <v-list two-line>
          <div>
            <v-list-tile
            class="food-slot"
              :key="item.title"
              avatar
              ripple
              v-for="item in getDrinks">
              <v-list-tile-content>
                <v-list-tile-title class="food-name">{{ item.Name }}</v-list-tile-title>
                <v-list-tile-sub-title class="portion-text">₦{{ item.Price }}</v-list-tile-sub-title>
                
              </v-list-tile-content>
            
              <v-list-tile-action>
               
                 <v-btn fab dark color="#f25d13" depressed class="meal-btn" @click="Tobasket(item)">
                <p>+</p>
                </v-btn>
              </v-list-tile-action>

            </v-list-tile>
      
          </div>
        </v-list>
      </v-card>

      </v-flex>
      <v-flex xs12 sm6 md6 class="hidden-sm-and-down">
        <v-card flat class="cart">
          <app-cart :Basket = 'basket' :Vendor = 'vendor'></app-cart>
        </v-card>
      </v-flex>
 <a class="bottom-icon" href="#main-meals"  v-smooth-scroll="{ duration: 1000, offset: -50 }">   
     <v-btn
              absolute
              dark
              fab
             bottom
              right
              color="#f25d13"
            >
              <v-icon>keyboard_arrow_up</v-icon>
            </v-btn></a> 
         </v-layout>
</div>
  
</template>

<script>
import db from './firebaseinit.js'
import { mapGetters } from 'vuex'
import Cart from './Cart.vue'
export default {
    data () {
      return {
       basket:[],
       vendor: 'ayo',
          rating: 4 ,
        selected: [2],
      }
    },
   computed:{
         ...mapGetters ([
             'getMain',
             'getSoup',
             'getSwallows',
             'getSides',
             'getDrinks'
         ]),
   },
    components: {
      'app-cart' : Cart
    },

    methods: {
      toggle (index) {
        const i = this.selected.indexOf(index)

        if (i > -1) {
          this.selected.splice(i, 1)
        } else {
          this.selected.push(index)
        }
      },
        
      Tobasket(item){
        this.basket.push({
          Name: item.Name,
          Price: item.Price,
          Quantity: 1
        })
      }

    }
}
</script>

<style scoped>
.food-slot {border-bottom: 1px solid rgb(241, 241, 241)}
.food-name {font-size: 15px;}
.bottom-icon {color:white;text-decoration: none;}
.tab-item {color: #f25d13;text-decoration: none; }
.vendor-header {font-size: 20px;padding-left:15px;}
.vendor {background:white;padding: 30px 100px 10px 100px;}
.cart {padding:20px 50px 20px 50px;}
.menu {padding:20px 0px 20px 0px;}
.meal-btn {width: 20px;height: 20px;}
.portion-text {color: rgb(209, 209, 209);font-size: 12px;}
 .res-owners-sec {margin-top:100px;text-align: center;}
      .res-header {font-size:35px;color: #ffffff;}
      .res-sub-header{font-size:14px;color: #ffffff;}
      .res-col-1{padding-top:60px;}
       .res-banner {
         padding: 100px 0px 0px 0px;
         margin: 0px 0px 20px 0px;
  height: 350px;
  text-align: center;
  background-image: -webkit-linear-gradient(270deg, rgba(252, 136, 41, 0.918), rgba(252, 136, 41, 0.918)), url('../assets/rees.jpg');
  background-image: linear-gradient(180deg,   rgba(252, 136, 41, 0.918),  rgba(252, 136, 41, 0.918)), url('../assets/rees.jpg');
  
  background-size: auto, cover;
  background-repeat: repeat, no-repeat;
  background-attachment: scroll, scroll;
  border-radius: 30px;
}

@media (max-width: 991px) {
 
}

@media (max-width: 767px) {


 
}

@media (max-width: 479px) {

.vendor-header {font-size: 20px;padding-left:15px;}
.vendor {background:white;padding: 3 0px 10px 10px 10px;}
.cart {padding:20px 50px 20px 50px;}
.menu {padding:20px 0px 20px 0px;}
.meal-btn {width: 20px;height: 20px;}
.portion-text {color: rgb(209, 209, 209);font-size: 12px;}
 .res-owners-sec {margin-top:100px;text-align: center;}
      .res-header {font-size:30px;color: #ffffff;}
      .res-sub-header{font-size:14px;color: #ffffff;}
      .res-col-1{padding-top:60px;}
       .res-banner {
         padding: 50px 20px 0px 20px;
         margin: 0px 0px 20px 0px;
  height: 250px;
  text-align: center;
  background-image: -webkit-linear-gradient(270deg, rgba(252, 136, 41, 0.918), rgba(252, 136, 41, 0.918)), url('../assets/rees.jpg');
  background-image: linear-gradient(180deg,   rgba(252, 136, 41, 0.918),  rgba(252, 136, 41, 0.918)), url('../assets/rees.jpg');
  
  background-size: auto, cover;
  background-repeat: repeat, no-repeat;
  background-attachment: scroll, scroll;
  border-radius: 30px;
}


}
</style>
